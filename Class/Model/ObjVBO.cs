...
class ObjVBO
{
    ...
    public void ObjToVBO(int vertexIndex, int textureIndex, string objFileName, string textureFileName)//Dependent on ParseObj
    {
        ...
        for(int i = 0; i < verticesAndFaceResult.Length; i++)
        {
            verticesAndFaceResult[i] = verticesAndFaceResult[i] * scale;
        }
        ...
        this.textureName = LoadTexture(textureFileName);
    }

    private int LoadTexture(string textureFileName)// GenTextures "out" cannot pass value outside class,
